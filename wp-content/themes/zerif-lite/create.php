<?php 
  ...
  $createmaptable = "CREATE TABLE IF NOT EXISTS maps (ID int AUTO_INCREMENT, PRIMARY KEY (ID),centerLat decimal (5,3),centerLong decimal (6,3),zoom tinyint)";
 
  if(!$result = $con->query($createmaptable)){
    die('dddddThere was an error running the query [' . $con->error . ']');
  }
  ...
  if(!$result = $con->query($createmappointtable)){
    die('There was an error running the querying [' . $con->error . ']');
  }
 $maps = array(
...
foreach ($maps as $ind) {
  ...
  if(!$insertmap = $con->query($newline)){
    die('There was an errors running the query [' . $con->error . ']');
  }
}
...
foreach ($mappoints as $indpt) {
  ...
  if(!$insertmap = $con->query($newline)){
    die('There was an ereeeeror running the query [' . $con->error . ']');
  }
} 
?>
